function [falsespk goodclust]= GroundTruthPerf(obj,gttimes)
    nbmaxunitperch=numel(unique(obj.Spkevent(6,obj.Spkevent(6,:)>0)));
    
    simulunit=1:size(gttimes,2);
    ...
    %for ch=1:nbCh
        for unitID = 1:nbmaxunitperch %Usel=1:nbmaxunitperch
            ...
            if sum(ismember(obj.SpkeventUM(6,:),Usel))>0 %>10%special condition for Mcdfiles with groundtruth
...
%                     
                    ...
                    spkSimul=gttimes{Usimul}'/obj.Params.fileinfo.samplingrate*10^6;
                    if numel(unique(obj.SpkeventUM(4,:))) == 1
                        spkEstim=obj.SpkeventUM(2,ismember(obj.SpkeventUM(6,:),Usel));% & obj.SpkeventUM(4,:)==2);
                    else
                        spkEstim=obj.SpkeventUM(2,ismember(obj.SpkeventUM(6,:),Usel) & obj.SpkeventUM(4,:)==1);
                    end
                    %precision=0.01*obj.Params.detect.Tcensoredfactor*obj.dwsamplefactor*size(obj.SpkwaveclustMulti{1},3)/obj.samplingrate*10^6;
                    precision=1*obj.Params.detect.Tcensoredfactor*obj.dwsamplefactor*size(obj.Spkwave,3)/obj.Params.fileinfo.samplingrate*10^6;
...
%                         %precision=0.01*obj.Params.detect.Tcensoredfactor*obj.dwsamplefactor*size(obj.SpkwaveclustMulti{1},3)/obj.samplingrate*10^6;
%                         precision=4*obj.Params.detect.Tcensoredfactor*obj.dwsamplefactor*size(obj.Spkwave,3)/obj.Params.fileinfo.samplingrate*10^6;
%                         NspkSimul=size(spkSimul,2);
%                         NspkEstim=size(spkEstim,2);
%                         for tspk=1:size(spkEstim,2)
%                             if ~isempty(spkSimul(spkSimul>=spkEstim(tspk)-precision & spkSimul<=spkEstim(tspk)+precision))
